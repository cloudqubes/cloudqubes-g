...
If you are installing vanilla Kubernetes on your laptop, installing on the host OS is discouraged. You get several advantages by using a virtual machine instead.
...
Update file permission so we can read this file without `sudo`.
...
Since we have not yet deployed any Workloads the Pods you see here are related to Kubernetes control plane.
...
Kubernetes depends on CNI plugins to implement network connectivity between Pods. There are several open-source network plugins but we'll use [Calico](https://www.tigera.io/project-calico/) for this Kubernetes cluster.
