...
![New version in production.](./canary-v2.png)
...
We are going to put this whole canary deployment into action in a Minikube cluster with the Ingress Nginx controller. 
...
The key here is using the ingress controller to split the traffic between the two versions. The Ingress Nginx controller uses `annotations` to define this traffic split. 

How you define this traffic split depends on the ingress controller as other ingress controllers may use different parameters but the underlying principles of canary release remains the same.

In a real-world production setup we will not be using `kubectl` to roll out a new version as it involves too much manual work. That's the job of the CI/CD pipeline.
